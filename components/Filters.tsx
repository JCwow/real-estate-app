...
const Filters = () => {
    ...
    <ScrollView horizontal showsVerticalScrollIndicator={false} className="mt-3 mb-2">
      {categories.map((item, index) => (
        <TouchableOpacity key={item.category} onPress={() => handleCategoryPress(item.category)} className={`flex flex-col items-start mr-4 px-4 py-2 rounded-full ${selectedCatagory === item.category ? 'bg-primary-300' : 'bg-primary-100 border border-primary-200'}`}>
